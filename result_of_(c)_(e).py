...
def best_of_N_Train(ppo_trainer, fix_LLM, tokenizer, generation_kwargs, N=10):
    ...
    for batch in tqdm(ppo_trainer.dataloader):
        ...
        for n in range(N):
          ...
          prediction = fix_LLM.generate(**response_encoded, max_new_tokens=100)
          # print(prediction)
...
def eval(ppo_trainer, fix_LLM, prompter_tokenizer, fixed_tokenizer, generation_kwargs, dataloader):
    ...
    #   print(response_encoded)         
        ...
        prediction = [pred.split("instruction: Question Answering")[-1] for pred in prediction]
        ...
        score += sum(rewards)
    return score / len(dataloader), prompts, predictions
...
if __name__ == "__main__":
    ...
    generation_kwargs = {
        ...
        "temperature": 0.6, # Need to modify
        "num_beams":1,
    ...
    max_len = 256
    temperature = 0.6  # 設定生成回覆的隨機度，值越小生成的回覆越穩定
    ...
    testing_score, generated_prompts, generated_response = eval(ppo_trainer, fix_LLM, prompter_tokenizer, fixed_tokenizer, generation_kwargs, test_dataloader)
    print("before RL testing_score:", testing_score)
    json.dump({"testing_score": testing_score, "generated_prompts": generated_prompts, "generated_response": generated_response}, open(f"(c).json", "w"), indent = 2, ensure_ascii=False)


    # Testing for RL
   
    sft_model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b-chat-hf", cache_dir="./cache", load_in_4bit=True, quantization_config=nf4_config,device_map='cuda:0') # peft_model_id is the path to sft adapters
    sft_model = PeftModel.from_pretrained(sft_model, "./ppo_add_eval_model_epoch_6")
    model = AutoModelForCausalLMWithValueHead.from_pretrained(sft_model, cache_dir="./cache", device_map='cuda:0').to("cuda:0")
    model.bfloat16()
    ...
    # Testing
    testing_score, generated_prompts, generated_response = eval(ppo_trainer, fix_LLM, prompter_tokenizer, fixed_tokenizer, generation_kwargs, test_dataloader)
    print(f"after RL testing_score: {testing_score}")

    json.dump({"generated_prompts": generated_prompts, "generated_response": generated_response}, open("(e).json", "w"), indent = 2, ensure_ascii=False)
